...
class Dataset_Visualization:
    ...
    def Plot_all(self,X,Y,title = 'All time series collected together',xtitle ='',ytitle =''
                 ,save = False,saveformat = '.pdf',path = 'images/'):
    ...
    def Plot_all_by_Class(self,X,Y,title = 'All time series sorting class by color',xtitle ='',ytitle =''
        ...
        train_dict = {}
        for key, i in zip(Y.unique(),Y_unique_class_values):
            train_dict[key] = i    
    ...
    def Plot_individual(self,X,Y,title = 'Time series in dataset number ',xtitle ='',ytitle =''
        ...
        for i in range(len(Y)):
            # fig_train = plt.figure("Figure " + str(i+1))
    ...
    def Plot_one(self,X,title = 'Time series in dataset number ',xtitle ='',ytitle =''
                     ,save = False,saveformat = '.pdf',path = 'images/',ylimite = None,savename= None):
        plt.plot(X, color="blue")
        plt.title(title )
        plt.xlabel(xtitle)
        plt.ylabel(ytitle)
        if ylimite is not None:
            plt.ylim(ylimite)
        if save:
            plt.savefig(path+savename+saveformat)
        plt.show()