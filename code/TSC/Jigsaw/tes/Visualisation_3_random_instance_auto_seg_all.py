...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun  2 10:39:38 2023
...
path_image_data = 'images/FCNEncoderDecoder/run4/jigsaw/variable3/'
    ...
    for permutation_num in range(1,seg_num+1):
        for bot in type_bottle:
            ...
            for data in datasets:
                ...
                predection_sample = predection[random_samples]



                min_y_value = min(np.min(original_sample),np.min(predection_sample))-0.1
                max_y_value = max(np.max(original_sample),np.max(predection_sample))+0.1


                for i in range(3):

                    plt.plot(original_sample[i], color="blue", label = "Original")
                    plt.plot(predection_sample[i], color="green", label = "Predection")
                    # plt.plot(predection_3_seg[i], color="red", label = "with bottleneck")
                    plt.legend()
                    plt.ylim(ymin = min_y_value,ymax = max_y_value)
                    plt.title(f'{data} with {seg_num} segments for {permutation_num} permutations \n and {bot} sample number {i}')
                    plt.savefig(path_image_data+'pdf/'+f'{data}_{i}_{seg_num}_segments_{permutation_num}_permutations_with_{bot}.pdf')
                    plt.savefig(path_image_data+'png/'+f'{data}_{i}_{seg_num}_segments_{permutation_num}_permutations_with_{bot}.png')
